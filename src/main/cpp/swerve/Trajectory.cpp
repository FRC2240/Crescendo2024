...
Trajectory::Trajectory(Drivetrain *drivetrain, Odometry *odometry,
  ...
  AutoBuilder::configureHolonomic(
      ...
      HolonomicPathFollowerConfig(
          PIDConstants(
              CONSTANTS::DRIVE::PATHPLANNER_TX.p,
              CONSTANTS::DRIVE::PATHPLANNER_TX.i,
              CONSTANTS::DRIVE::PATHPLANNER_TX.d), // Translation PID constants
          PIDConstants(
              CONSTANTS::DRIVE::PATHPLANNER_RT.p,
              CONSTANTS::DRIVE::PATHPLANNER_RT.i,
              CONSTANTS::DRIVE::PATHPLANNER_RT.d), // Rotation PID constants
...
frc2::CommandPtr Trajectory::auto_score_align() {
  return frc2::cmd::RunOnce([this] { cyclecounter = 0; }, {this})
      .AndThen(
          frc2::cmd::Run(
              [this] {
                ...
                if (frc::DriverStation::GetAlliance().has_value() &&
                  ...
                  speakerpose =
                      frc::Pose2d(0.22_m, 5.5_m, frc::Rotation2d(0_rad));
                } else {
                  speakerpose = frc::Pose2d(16.9_m, 5.5_m,
                                            frc::Rotation2d(0_deg)); // CHANGEME
                }
                ...
                // if (frc::DriverStation::GetAlliance().has_value() &&
                // frc::DriverStation::GetAlliance().value() ==
                // frc::DriverStation::Alliance::kRed){ desired_angle =
                // units::degree_t{(atan2(botpose.Y().value(),botpose.X().value())/std::numbers::pi*180)}+180_deg;
                // }
                //     desired_angle += units::degree_t{180};
                //   } else {
                //     desired_angle -= units::degree_t{180};
              ...
              .Until([this] -> bool {
                ...
                if (CONSTANTS::IN_THRESHOLD<units::degree_t>(
                        m_drivetrain->getAngle(), desired_angle, .1_deg)) {
                  cyclecounter++;
                }
