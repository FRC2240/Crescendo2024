...
Trajectory::Trajectory(Drivetrain *drivetrain, Odometry *odometry,
                       frc::XboxController *stick, Vision *vision,
                       Intake *intake)
    : m_drivetrain{drivetrain}, m_odometry{odometry}, m_stick{stick},
      m_vision{vision}, m_intake{intake} {
  AutoBuilder::configureHolonomic(
      ...
      HolonomicPathFollowerConfig(
          ...
          17.324_in, // Drive base radius in meters. Distance from robot center
                     // to furthest module.
  ...
  return frc2::cmd::Run(
      [this, field_relative] {
        ...
        const units::meters_per_second_t left_right{
            frc::ApplyDeadband(m_stick->GetLeftX(), 0.1) *
            CONSTANTS::DRIVE::TELEOP_MAX_SPEED};
        const units::meters_per_second_t front_back{
            frc::ApplyDeadband(m_stick->GetLeftY(), 0.1) *
            CONSTANTS::DRIVE::TELEOP_MAX_SPEED};
        auto const rot = frc::ApplyDeadband(m_stick->GetRightX(), .1) *
...
frc2::CommandPtr Trajectory::make_relative_line_path(units::meter_t x,
                                                     units::meter_t y,
                                                     frc::Rotation2d rot) {

  return frc2::cmd::DeferredProxy([this, x, y, rot] {
           auto pose = m_odometry->getPose();
           // fmt::println("Current Pose: {},{},{}", pose.X().value(),
           ...
           // fmt::println("Target Pose: {},{},{}", (pose.X() + x).value(),
           // (pose.Y() + y).value(), rot.Degrees().value());

           std::vector<frc::Translation2d> bezierPoints =
               PathPlannerPath::bezierFromPoses(points);
           auto path = std::make_shared<PathPlannerPath>(
               bezierPoints, DEFAULT_CONSTRAINTS, GoalEndState(0.0_mps, rot));

           return AutoBuilder::followPathWithEvents(path).AndThen(
               frc2::PrintCommand("here").ToPtr());
         })
...
frc2::CommandPtr Trajectory::make_absolute_line_path(frc::Pose2d target_pose) {
  return frc2::PrintCommand("start")
      .ToPtr()
      .AndThen(AutoBuilder::pathfindToPose(target_pose, DEFAULT_CONSTRAINTS,
                                           1_mps, 0.0_m))
      .AndThen(frc2::PrintCommand("end").ToPtr());
}
...
frc2::CommandPtr Trajectory::auto_score_align() {
  std::function<void()> init =
      [this]() { /*no data currently needed */
                 frc::SmartDashboard::PutBoolean("shooter/is auto aligning",
                                                 false);
      };
  std::function<void()> periodic = [this]() {
    try {
      ...
      if (angle) {
        ...
        frc::SmartDashboard::PutNumber("auto score bot angle",
                                       angle.value().value());
        m_drivetrain->face_direction(0_deg, -angle.value().value());
      }
    } catch (const std::exception &e) {
  ...
  std::function<bool()> is_finished = [this]() -> bool {
    ...
    if (angle) {
      return (CONSTANTS::IN_THRESHOLD<units::degree_t>(angle.value(), 0_deg,
                                                       5_deg));
    }
  };
  ...
  frc2::CommandPtr Trajectory::auto_score_align() {
    return frc2::cmd::Run(
        ...
        frc2::CommandPtr Trajectory::make_relative_line_path(
            units::meter_t x, units::meter_t y, frc::Rotation2d rot) {
          return frc2::cmd::DeferredProxy([this, x, y, rot] {
                   auto pose = m_odometry->getPose();
                   // fmt::println("Current Pose: {},{},{}", pose.X().value(),
                   // pose.Y().value(), pose.Rotation().Degrees().value());
                   ...
                   // fmt::println("Target Pose: {},{},{}", (pose.X() +
                   // x).value(), (pose.Y() + y).value(),
                   // rot.Degrees().value());

                   std::vector<frc::Translation2d> bezierPoints =
                       PathPlannerPath::bezierFromPoses(points);
                   auto path = std::make_shared<PathPlannerPath>(
                       bezierPoints, DEFAULT_CONSTRAINTS,
                       GoalEndState(0.0_mps, rot));
        ...
        frc2::CommandPtr Trajectory::make_absolute_line_path(
            frc::Pose2d target_pose) {
          return frc2::PrintCommand("start")
              .ToPtr()
              .AndThen(AutoBuilder::pathfindToPose(
                  target_pose, DEFAULT_CONSTRAINTS, 1_mps, 0.0_m))
              .AndThen(frc2::PrintCommand("end").ToPtr());
        }
        ...
        frc2::CommandPtr Trajectory::auto_pickup() {
          ...
          std::function<void()> periodic = [this]() {
            try {
              std::optional<units::degree_t> angle =
                  m_vision->get_coral_angle();
              if (angle) {
                frc::SmartDashboard::PutString("coral intake state",
                                               "target locked");
                frc::SmartDashboard::PutNumber("ima sicko mode",
                                               angle.value().value());
                m_drivetrain->face_direction(0_deg, angle.value().value());
                frc::DataLogManager::Log("3");
                if (angle.value() < CONSTANTS::INTAKE::AUTO_PICKUP_THRESHOLD &&
          ...
          std::function<void(bool IsInterrupted)> end =
              [this](bool IsInterrupted) { frc::DataLogManager::Log("end"); };
          return frc2::FunctionalCommand(init, periodic, end, is_finished,
                                         {this})
              .ToPtr();
        }
    ...
    return frc2::cmd::Run([this]
        ...
        if (frc::DriverStation::GetAlliance().has_value() &&
          ...
          speakerpose =
              frc::Pose2d(0_m, 5.5_m, frc::Rotation2d(0_rad)); // CHANGEME
        } else {
          speakerpose = frc::Pose2d(16.46_m, 5.5_m,
                                    frc::Rotation2d(3.1415_rad)); // CHANGEME
        }
        botpose = botpose.RelativeTo(speakerpose);
        // angle of relative vector
        desired_angle =
            units::degree_t{-(atan2(botpose.Y().value(), botpose.X().value()) /
                              std::numbers::pi * 180)};
        /*
        ...
        frc::SmartDashboard::PutNumber("as/rel t",
                                       botpose.Rotation().Degrees().value());
        frc::SmartDashboard::PutNumber("as/desired angle",
                                       desired_angle.value());
