...
{
  return frc2::PrintCommand("start four gamepiece pos_1").ToPtr().AndThen(traj->extract("a_pos_2_gp1").AndThen(frc2::PrintCommand("one piece completed").ToPtr()));
}

frc2::CommandPtr autos::midline_middle(Trajectory *traj)
{
  return frc2::PrintCommand("start four gamepiece pos_1").ToPtr().AndThen(traj->extract("a_sourcide_gp2v3").AndThen(frc2::PrintCommand("three piece completed").ToPtr()));
}

frc2::CommandPtr autos::midline_3gp(Trajectory *traj)
{
...
frc2::CommandPtr autos::midline_2gp_blue(Trajectory *traj)
{
  return frc2::PrintCommand("start four gamepiece pos_1").ToPtr().AndThen(traj->extract("a_sourcide_gp2v2_blue").AndThen(frc2::PrintCommand("four piece completed").ToPtr()));
}
