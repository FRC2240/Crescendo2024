...
frc2::CommandPtr Coral::TrackCommand() {
    return frc2::RunCommand([this] -> void {
        ...
        units::degree_t rel_x = units::degre_t{m_table->GetNumber("tx", 0.0)};
        units::degree_t rel_z = units::degre_t{m_table->GetNumber("ty", 0.0)}

        //compute absolute angle of note
        //maybe try get_absolute_angle()?
        units::degree_t abs_x = m_drivetrain->getAngle() + tx;
        ...
        m_drivetrain->faceDirection(dx, dy, abs_x);
