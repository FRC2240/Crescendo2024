...
Intake::Intake() {
  ...
  angle_config.MotorOutput.NeutralMode =
  ...
  belt_config.MotorOutput.NeutralMode =
      ctre::phoenix6::signals::NeutralModeValue::Brake;
...
frc2::CommandPtr Intake::zero() {
  return frc2::cmd::RunOnce([this] { m_angleMotor.SetPosition(0_tr); }, {this});
}

bool Intake::is_loaded() { return m_tof.GetRange() < 150; };

bool Intake::is_lower_tof_loaded() { return m_lower_tof.GetRange() < 150; };
             ...
             [this] -> void {
               ...
               m_angleMotor.SetControl(
                   ctre::phoenix6::controls::PositionDutyCycle{
                       CONSTANTS::INTAKE::DOWN_POSITION});
             },
             {this})
      .Until([this] -> bool // stop when done (to allow StartSpinCommand to run)
             {
               return CONSTANTS::IN_THRESHOLD<units::turn_t>(
                   m_angleMotor.GetPosition().GetValue(),
                   CONSTANTS::INTAKE::DOWN_POSITION,
                   CONSTANTS::INTAKE::ROTATION_THRESHOLD);
             })
      .WithName("Extend");
};
...
frc2::CommandPtr Intake::RetractCommand() {
  return frc2::RunCommand(
             [this] -> void {
               m_angleMotor.SetControl(
                   ctre::phoenix6::controls::PositionDutyCycle{
                       CONSTANTS::INTAKE::UP_POSITION});
             },
             {this})
      .Until(
          [this] -> bool { // stop when done (to allow StopSpinCommand to run)
            return CONSTANTS::IN_THRESHOLD<units::turn_t>(
                m_angleMotor.GetPosition().GetValue(),
                CONSTANTS::INTAKE::UP_POSITION,
                CONSTANTS::INTAKE::ROTATION_THRESHOLD);
...
frc2::CommandPtr Intake::StartSpinCommand() {
  return frc2::InstantCommand(
    ...
    m_beltMotor.SetControl(
        ctre::phoenix6::controls::VoltageOut(m_belt_velocity));
                   },
                      is_intaking = true;
                      m_beltMotor.SetControl(
                          ctre::phoenix6::controls::VoltageOut(units::volt_t{
                              CONSTANTS::INTAKE::INTAKE_VOLTAGE}));
                      is_intaking = true;
                      m_beltMotor.SetControl(
    ...
    return false;
    .AndThen(StopSpinCommand()).WithName("StartSpin");
};
...
frc2::CommandPtr Intake::StopSpinCommand() {
    return frc2::RunCommand(
               [this] {
                 ...
                 m_beltMotor.SetControl(ctre::phoenix6::controls::VoltageOut{
                     units::voltage::volt_t{0}}); // wrong type?
               },
               {this})
        .Until([this] -> bool {
...
frc2::CommandPtr Intake::Wes() {
    return frc2::cmd::Run([this] { m_beltMotor.Set(0.3); }, {this})
        .Until([this] -> bool {
          if (is_loaded()) {
            m_timer.Start();
          }
