...
Shooter::Shooter(Odometry *odometry, Intake *intake)
    : m_odometry{odometry}, m_intake{intake} {
  ...
  right_conf.MotorOutput.Inverted =
      right_conf.MotorOutput.Inverted.Clockwise_Positive;
  ...
  belt_conf.MotorOutput.NeutralMode =
...
void Shooter::Periodic() {
  if (m_intake->intake_state == Intake::IntakeState::INTAKING) {
    ...
    m_belt_motor.SetControl(
        ctre::phoenix6::controls::VelocityVoltage(0_tr / 1_s));
...
frc2::CommandPtr Shooter::default_cmd() {
  return frc2::RunCommand(
             [this] {
               ...
               //        m_belt_motor.SetControl(ctre::phoenix6::controls::VelocityVoltage(0_tr
               //        / 1_s)); frc::SmartDashboard::PutBoolean("intaking",
               //        0);
               //    }
...
frc2::CommandPtr Shooter::test_shot() {
  ...
  std::function<void()> periodic = [this] {
    units::turn_t angle =
        units::turn_t{frc::SmartDashboard::GetNumber("shooter/dangle", 0.0)};
    // fmt::println("{}", angle.value());
    set_angle(angle);
    units::turns_per_second_t vel{
        frc::SmartDashboard::GetNumber("shooter/desired velocity", 0.0)};
    // m_left_motor.SetControl(ctre::phoenix6::controls::VelocityVoltage(vel));
    // m_right_motor.SetControl(ctre::phoenix6::controls::VelocityVoltage(vel *
    // 0.8));
  ...
  std::function<bool()> is_finished = [this] -> bool {
    ...
    frc::SmartDashboard::PutNumber(
        "shooter/speed", m_left_motor.GetVelocity().GetValue().value());
    frc::SmartDashboard::PutBoolean("shooter/speed_threshold",
                                    m_left_motor.GetVelocity().GetValue() >
                                        75_tps);
    frc::SmartDashboard::PutBoolean(
        "shooter/angle_threshold",
        CONSTANTS::IN_THRESHOLD<units::angle::degree_t>(
            get_angle(),
            units::turn_t{
                frc::SmartDashboard::GetNumber("shooter/dangle", 0.0)},
            0.25_tr));
...
frc2::CommandPtr Shooter::fender_shot() {
  ...
  std::function<void()> periodic = [this] {
    ...
    m_left_motor.SetControl(
        ctre::phoenix6::controls::VoltageOut(units::volt_t{12}));
    m_right_motor.SetControl(
        ctre::phoenix6::controls::VoltageOut(units::volt_t{6}));
    // m_right_motor.SetControl(ctre::phoenix6::controls::VelocityDutyCycle(-CONSTANTS::SHOOTER::RIGHT_VELOCITY));
  };
  std::function<bool()> is_finished = [this] -> bool {
    if (frc::DriverStation::IsAutonomous()) {
      frc::SmartDashboard::PutNumber("shooter/turns",
                                     get_angle().value() / 360);
      frc::SmartDashboard::PutNumber(
...
frc2::CommandPtr Shooter::set_angle_cmd(std::optional<units::degree_t> angle) {
  ...
  std::function<bool()> is_finished = [this, &angle]() -> bool {
    try {
      if (angle) {
        return CONSTANTS::IN_THRESHOLD<units::turn_t>(get_angle(),
                                                      angle.value(), 1_deg);
      } else {
        return false;
      }
...
frc2::CommandPtr Shooter::set_angle_cmd(units::turn_t angle) {
  std::function<void()> init = [this] {};
  std::function<void()> periodic = [this, &angle]() { set_angle(angle); };

  std::function<bool()> is_finished = [this, &angle]() -> bool {
    auto ret =
        CONSTANTS::IN_THRESHOLD<units::degree_t>(get_angle(), angle, 1_deg);
  ...
  return frc2::cmd::Run(
             [this, angle] {
               frc::SmartDashboard::PutNumber(
                   "shooter/ yet another stupid angle", angle.value());
               set_angle(angle);
             },
             {this})
      .WithTimeout(0.5_s)
...
frc2::CommandPtr Shooter::amp_shot() {
  ...
  std::function<void()> periodic = [this] {
    units::turn_t angle =
        units::turn_t{frc::SmartDashboard::GetNumber("amp/dangle", 0.0)};
    // units::turn_t angle = units::turn_t{11};
    // units::turn_t angle = units::turn_t{9};
    // units::volt_t vout = units::volt_t{-4};
    ctre::phoenix6::controls::VelocityVoltage req = units::turns_per_second_t{
        frc::SmartDashboard::GetNumber("amp/desired velocity", 0.0)};
    // ctre::phoenix6::controls::VelocityVoltage req =
    // units::turns_per_second_t{-6.5};
    set_angle(angle);
  ...
  std::function<bool()> is_finished = [this] -> bool {
    frc::SmartDashboard::PutNumber("shooter/turns", get_angle().value() / 360);
    frc::SmartDashboard::PutNumber(
        "shooter/lvelocity", m_left_motor.GetVelocity().GetValueAsDouble());
    frc::SmartDashboard::PutNumber(
        "shooter/rvelocity", m_right_motor.GetVelocity().GetValueAsDouble());
    return CONSTANTS::IN_THRESHOLD<units::angle::degree_t>(
               get_angle(), CONSTANTS::SHOOTER::AMP_ANGLE, 0.25_tr) &&
           CONSTANTS::IN_THRESHOLD<units::turns_per_second_t>(
               m_left_motor.GetVelocity().GetValue(),
               ...
               0.25_tps) &&
           CONSTANTS::IN_THRESHOLD<units::turns_per_second_t>(
               m_right_motor.GetVelocity().GetValue(),
               units::turns_per_second_t{
                   frc::SmartDashboard::GetNumber("amp/desired velocity", 0.0)},
               0.25_tps);
    // change threshold?
    //    &&
    //    CONSTANTS::IN_THRESHOLD<units::angular_velocity::turns_per_second_t>(m_left_motor.GetVelocity().GetValue(),
    //    CONSTANTS::SHOOTER::LEFT_VELOCITY,
    //    units::angular_velocity::turns_per_second_t{1})    // change
    //    threshold?
    //    &&
    //    CONSTANTS::IN_THRESHOLD<units::angular_velocity::turns_per_second_t>(m_right_motor.GetVelocity().GetValue(),
    //    CONSTANTS::SHOOTER::RIGHT_VELOCITY,
...
frc2::CommandPtr Shooter::zero() {
  return frc2::cmd::RunOnce([this] { m_angle_motor.SetPosition(0_tr); },
                            {this});
}

frc2::CommandPtr Shooter::intake_cmd() {
  return frc2::cmd::Run([this] { m_belt_motor.Set(.25); }, {this});
}

void Shooter::intake() { m_belt_motor.Set(.25); }
...
frc2::CommandPtr Shooter::execute_auto_shot() {
  ...
  std::function<bool()> is_finished = [this] -> bool {
    return CONSTANTS::IN_THRESHOLD<units::angle::degree_t>(
               get_angle(), m_odometry->get_shooter_angle(), 0.25_tr) &&
           m_left_motor.GetVelocity().GetValue() < -60_tps;
  };
  std::function<void(bool IsInterrupted)> end = [this](bool IsInterrupted) {};
